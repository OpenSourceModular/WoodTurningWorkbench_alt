...
class BowlConstructionLines:
	def GetResources(self):
		...
		return {
			'Pixmap': str(Path(FreeCAD.getUserAppDataDir()) / "Mod" / "WoodturningWorkbench_alt" / "resources" / "icons" / "BowlConstructionLines.svg"),
		...
		class AddBowlConstructionLines:
			...
			def __init__(self):
				...
				if not doc.getObject("BowlVariables"):
					...
					self.bowl_radius = round(getVarsetValue(self,"BowlWidth") / 2 , 2)
