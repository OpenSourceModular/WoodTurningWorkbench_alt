...
class AddSegments:
	...
	def GetResources(self):
		...
		import FreeCAD
		return {
			'Pixmap': str(Path(FreeCAD.getUserAppDataDir()) / "Mod" / "WoodturningWorkbench_alt" / "resources" / "icons" / "AddSegments.svg"),
	...
	def Activated(self):
		...
		class AddSegmentsTaskPanel:
			...
			def __init__(self):
				...
				sub_button_layout1 = QtWidgets.QVBoxLayout()
				self.hide_segments_cb = QtGui.QCheckBox("Hide Segments")
				self.hide_segments_cb.stateChanged.connect(lambda: self.bt_show_hide("Segment"))
				sub_button_layout1.addWidget(self.hide_segments_cb)
				self.hide_segments_cb.setChecked(False)

				sub_button_layout2 = QtWidgets.QVBoxLayout()
				self.hide_bowl_solid_cb = QtGui.QCheckBox("Hide Bowl Solid")
				self.hide_bowl_solid_cb.stateChanged.connect(lambda: self.bt_show_hide("BowlSolid"))
				sub_button_layout2.addWidget(self.hide_bowl_solid_cb)
				self.hide_bowl_solid_cb.setChecked(False)

				sub_button_layout3 = QtWidgets.QVBoxLayout()
				self.hide_intersects_cb = QtGui.QCheckBox("Hide Intersects")
				self.hide_intersects_cb.stateChanged.connect(lambda: self.bt_show_hide("Intersect"))
				sub_button_layout3.addWidget(self.hide_intersects_cb)
				self.hide_intersects_cb.setChecked(False)

				button_layout10.addLayout(sub_button_layout1)
				button_layout10.addLayout(sub_button_layout2)
				button_layout10.addLayout(sub_button_layout3)


				button_layout11 = QtWidgets.QHBoxLayout()
				# Cancel button
				self.cancel_button = QtWidgets.QPushButton("Close")
				self.cancel_button.clicked.connect(self.on_cancel)
				button_layout11.addWidget(self.cancel_button)
				...
				layout.addLayout(button_layout11)
			...
			def bt_show_hide(self, object_type):
				doc = App.ActiveDocument
				print(f"Toggling visibility for {object_type}")
				for obj in doc.Objects:
					if object_type in obj.Label:
			...
			def bt_edit_segment_click(self, radius_type="inner", direcion="decrease"):
				...
				print(f"Selected segment: {seg_name}")
			...
			def	bt_intersect_segments_click(self):
				...
				for a in range(0,len(intersection_list)):
					...
					g_ = 81/256
					b_ = 68/256
					o.ViewObject.ShapeColor = (r_, g_, b_)
					o.Shape=p
					del p, o
				doc.recompute()
